...
카이사르는 비밀스럽게 편지를 쓸 때, 'A'를 'D로', 'B'를 'E'로, 'C'를 'F'로... 이런 식으로 알파벳 문자를 3개씩 건너뛰어 적었다고 합니다.
...
a = [chr(i) for i in range(ord('D'), ord('Z')+1) ] + [chr(i) for i in range(ord('A'), ord('C')+1) ]

s = str(input())
for c in s:
    print(b[a.index(c)], end="")
